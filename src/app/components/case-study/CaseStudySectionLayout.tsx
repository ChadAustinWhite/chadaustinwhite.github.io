...
import { useCaseStudySectionBrand } from './CaseStudySectionBrandContext';
...
export function CaseStudySectionHeader({ sectionLabel }: { sectionLabel: string }) {
  const brandLabel = useCaseStudySectionBrand();
  return (
...
const bodyTextClass =
  'space-y-4 text-[15px] font-normal leading-[1.65] text-[var(--ink)] md:text-[17px]';
...
export function CaseStudySectionLayout({
  ...
  return (
    <>
      {showSectionHeader ? <CaseStudySectionHeader sectionLabel={sectionLabel} /> : null}
      ...
      {showHeadlineColumn ? (
        <div className="mt-10 grid grid-cols-1 gap-8 md:grid-cols-12 md:gap-12">
          ...
          <div className={`md:col-span-7 ${bodyTextClass}`}>
            {body.map((p, i) => (
              <p key={i}>{p}</p>
            ))}
          </div>
